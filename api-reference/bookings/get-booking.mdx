...
<ParamField path="id" type="integer" required>
  The unique integer identifier of the booking to retrieve. Obtain this value
  from the `id` field on any Booking object returned by `GET /bookings`.
</ParamField>
...
<Note>
  Requesting an id that belongs to a booking in another user's account returns
  `404`, not `403`. The API never confirms or denies that a booking outside your
  account exists, so you cannot enumerate other users' booking ids through error
  codes.
</Note>
